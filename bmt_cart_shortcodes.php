<?php
	...
	add_shortcode('bmt_add_show_cart_button', 'bmt_add_show_cart_handler');
	...
	function bmt_add_show_cart_handler($atts) {
		extract(shortcode_atts(array(
			'product_id' => '',
		), $atts));
		if(empty($product_id)){
		...
		return print_bmt_add_show_cart_button_for_product($product_id, $atts);
	}
